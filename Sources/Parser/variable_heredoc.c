...
char	*is_variable_in_env_bis(char *var, t_env *env)
{
	int	i;
...
int	size_env_var_bis(char *all_env_var, int *j)
{
	int	i;
	int size;
...
char	*fill_env_var_bis(char *all_env_var)
{
	int		i;
	int		j;
	int		size;
	...
	i = 0;
	j = 0;
	size = size_env_var_bis(all_env_var, &j);
...
bool	is_there_sign_before_bis(char *lex_string, int j)
{
	j = j - 1;
	while (j >= 0)
	{
...
int	size_new_arg_bis(char *lex_string, int count, t_env *env)
{
	int	j;
	int	k;
	int size;
	...
	while (k < count)
	{
		...
		if (var[0] != '?')
		{
			all_env_var = is_variable_in_env_bis(var, env);
			if (all_env_var)
				size = size + size_env_var_bis(all_env_var, NULL);
		}
		else
		{
...
int	nbr_dollar_sign_bis(char *lex_string)
{
	int	size;
	int count;
	count = 0;
...
char	*handle_var_bis(char *lex_string, t_env *env)
{
	...
	e = 0;
	sizearg = 0;
	count = nbr_dollar_sign_bis(lex_string);
	sizearg = size_new_arg_bis(lex_string, count, env);
	...
	while (k < count && sizearg != 0)
	{
		...
		if (var[0] != '?')
		{
			...
			if (all_env_var)
			{
				env_var = fill_env_var_bis(all_env_var);
				if (!env_var)
					return (NULL);
				i = 0;
				while (env_var[i])
